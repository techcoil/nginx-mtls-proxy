...
if [ -n "$MTLS_CA_CERT_PATH" ] && [ -f "$MTLS_CA_CERT_PATH" ]; then
    echo "Using CA certificate: $MTLS_CA_CERT_PATH"
    ...
    if [ "$MTLS_VERIFY_CERT" != "off" ]; then
        echo "Enabling certificate verification: $MTLS_VERIFY_CERT"
        MTLS_VERIFY_CONFIG="proxy_ssl_verify $MTLS_VERIFY_CERT;"
    fi
else
    echo "No CA certificate configured or file not found. SSL verification disabled."
